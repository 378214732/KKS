...
//        return \think\facade\Cache::store('redis');
    ...
    {
        $this->cacheTag($tag)->clear();
    }

    /**
     * 删除单个缓存
     */
    public function cacheLogicDel(string $key = '')
    {
        return Cache::redisHandler()->delete($this->cacheKey().'tag'.$key);
    }
    ...
    {
        return 'KSS:' . md5(Env::get('kss.app_key')) . substr($this->setModel(), strrpos($this->setModel(), '\\') + 1) . ':';
    }
