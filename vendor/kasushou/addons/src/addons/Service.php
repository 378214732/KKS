...
class Service extends \think\Service
{
    ...
    public function register()
    {
        ...
        $this->loadService();
        // 4.自动加载全局的插件内部第三方类库
        addons_vendor_autoload($this->addons_data_list ? $this->addons_data_list : Cache::redisHandler()->get('addons_data_list', []));
...
//
//
//                    foreach ($val as $k => $rule) {
//                        list($addon, $controller, $action) = explode('/', $rule['rule']);
//                        $route->rule($rule['key'], $execute)
//                            ->name($rule['name'] ?? '插件路由')
//                            ->completeMatch(true)
//                            ->append([
//                                'addon' => $addon,
...
//        });
    ...
    private function loadLang()
    {
        // 加载系统语言包
        Lang::load([
            $this->app->getRootPath() . '/vendor/kasushou/addons/src/lang/zh-cn.php'
        ]);
        // 加载应用默认语言包
    ...
    private function loadService()
    {
        ...
        foreach ($results as $name) {
            ...
            $info = parse_ini_file($service_file, true, INI_SCANNER_RAW) ?: [];
            if ($info) {
    ...
    {
        $hooks = $this->app->isDebug() ? [] : Cache::redisHandler()->get('hooks', []);
        if (empty($hooks)) {
            ...
            Cache::redisHandler()->set('hooks', $hooks);
        }
    ...
    private function autoload()
    {
        ...
        // 读取插件目录及钩子列表
        $base = get_class_methods("\\kasushou\\Addons");
        ...
        foreach (glob($this->getAddonsPath() . '*/*.php') as $addons_file) {
            ...
            if (strtolower($info['filename']) === 'plugin') {
                ...
                $addon_config = parse_ini_file($ini, true, INI_SCANNER_RAW) ?: [];
...
//                $common_file = $info['dirname'] . DS . 'common.php';
...
//                $route_file = $info['dirname'] . DS . 'route.php';
...
//            }
        ...
        //插件配置信息保存到缓存
        Cache::redisHandler()->set('addons_config', $config);
        //插件列表
        Cache::redisHandler()->set('addons_data', $this->addons_data);
        //插件ini列表
        Cache::redisHandler()->set('addons_data_list', $this->addons_data_list);
        //插件config列表
        Cache::redisHandler()->set('addons_data_list_config', $this->addons_data_list_config);
    ...
    {
        $assetsDir = app()->getRootPath() . str_replace("/", DS, "public");
        if (!is_dir($assetsDir)) {
    ...
    public static function copyApp($name, $delete = false)
    {
        ...
        foreach (Service::getAppDir($name) as $k => $dir) {
            $sourcedir = Service::getAddonsNamePath($name) . $dir . DS;
            if (is_dir($sourcedir)) {
                FileHelper::copyDir($sourcedir, app()->getBasePath() . DS, $delete);
...
//        $appDir = app()->getBasePath() . $name;
        ...
        if ($delete) FileHelper::delDir($addonPath);
...
//        }
    ...
    public static function updateAddonsInfo($name, $state = 1, $install = 1)
    {
        ...
        Cache::redisHandler()->set('addonslist', $addonslist);
