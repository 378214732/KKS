...
class SwooleHandler
{
    ...
    public function __invoke(RequestInterface $request, array $options)
    {
        ...
        if (isset($options['allow_redirects']) && $options['allow_redirects'] === false) {
            // 禁止重定向
            $yurunRequest = $yurunRequest->withAttribute(Attributes::FOLLOW_LOCATION, false);
        }
        //fonnie 设置cookie
        if ($yurunRequest->hasHeader('Cookie')) {
            try {
                $cookies = $yurunRequest->getHeader('Cookie')[0];
                $cookies = explode(';', $cookies);
                $values = [];
                foreach ($cookies as $cookie) {
                    $cookie = trim($cookie);
                    $num = stripos($cookie, '=');
                    $key = substr($cookie, 0, $num);
//                    $val = urldecode(substr($cookie, $num + 1)) ;
                    $val = substr($cookie, $num + 1);
                    $values[$key] = $val;

                }
//                dump($values);
                $yurunRequest = $yurunRequest->withCookieParams($values);
//                dump($yurunRequest->getHeaders());
            } catch (\Throwable $e) {
...
//            dump($request->getBody());
            ...
            if (false !== stripos((string)$yurunRequest->getHeader('Content-Type')[0], 'multipart/form-data')) {

//                try {
//                    parse_str($request->getBody(), $reqData);
//                } catch (\Throwable $e) {
//                }
////                file_put_contents('2.txt', $request->getBody() . "\r\n", 8);
//                dump($reqData);
//                if (!empty($reqData['fonniefile'])) {
//                    dump($reqData);
////                    $yurunRequest =  $yurunRequest->with($reqData);
////                    $requestBody = $yurunRequest->getBody();
//
//                    $files = [];
//                    foreach ($reqData['fonniefile'] as $formItem) {
//                        //文件
//                        if (!empty($formItem['filename'])) {
//                            $files[$formItem['name']] = [
//                                'name' => $formItem['filename'],
//                                'type' => $formItem['type'] ?? '',
//                                'tmp_name' => $formItem['contents'] ?? '',
//                                'size' => $formItem['size'] ?? null,
//                                'error' => $formItem['error'] ?? 0,
//                            ];
//                        }
//                    }
//
//                    dump($files);
//                    $yurunRequest = $yurunRequest->withUploadedFiles($files);
//
//                }


//                dump(($request->getBody())->read());
//                if (!empty($options['multipart'])) {
//                    $files = [];
//                    foreach ($options['multipart'] as $formItem) {
//                        //文件
//                        if (!empty($formItem['filename'])) {
//                            $files[$formItem['name']] = [
//                                'name' => $formItem['filename'],
